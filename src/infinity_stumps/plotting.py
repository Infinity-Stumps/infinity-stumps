...
from .geometry import ANCHORS_8, PITCH_HL, STUMP_HEIGHT, STUMP_W
...
def draw_pitch_topdown(ax, anchors=ANCHORS_8, show_anchor_labels=True):
    ...
    for x_end in [-PITCH_HL, PITCH_HL]:
        ax.add_patch(
            patches.Rectangle(
                (x_end - 0.03, -STUMP_W / 2), 0.06, STUMP_W, color="saddlebrown"
            )
        )
    ax.scatter(
        anchors[:, 0],
    ...
    ax.set_xlim(-12, 12)
    ax.set_ylim(-3, 3)
    ax.set_xlabel("X along pitch (m)")
    ax.set_ylabel("Y across pitch (m)")
    ax.grid(alpha=0.3)


def draw_pitch_sideview(ax, anchors=ANCHORS_8):
    ax.scatter(
        anchors[:, 0], anchors[:, 2], s=120, c="steelblue", edgecolors="k", zorder=5
    )
    for x_end in [-PITCH_HL, PITCH_HL]:
