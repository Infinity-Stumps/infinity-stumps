...
Mirrors DRS / Hawk-Eye logic on Cricket-UWB hardware + iPhone compute.
...
def detect_impact(times: NDArray[np.float64],
...
def extrapolate_to_stumps(release_pos: NDArray[np.float64],
    ...
    """
    ts, st = integrate_trajectory(release_pos, v0, spin,
...
def fit_covariance(jac: NDArray[np.float64],
...
def stump_line_covariance(params: NDArray[np.float64],
    ...
    J = np.zeros((2, n))
    for i in range(n):
        dp = np.zeros(n); dp[i] = eps
        plus = extrapolate_to_stumps(params[:3] + dp[:3],
                                       params[3:6] + dp[3:6],
                                       params[6:9] + dp[6:9],
                                       stump_x, params=ball_params)
        minus = extrapolate_to_stumps(params[:3] - dp[:3],
                                        params[3:6] - dp[3:6],
                                        params[6:9] - dp[6:9],
                                        stump_x, params=ball_params)
...
def lbw_verdict(yz_predicted: NDArray[np.float64],
    ...
    pts_y = y + c_*ex - s_*ey
    pts_z = z + s_*ex + c_*ey
    inside = ((np.abs(pts_y) <= stump_half_w)
              & (pts_z >= 0.0) & (pts_z <= stump_top))
...
def find_bounce(release_pos, v0, spin,
                params: BallParams | None = None,
                t_max: float = 2.0) -> NDArray[np.float64] | None:
...
def assess_lbw(release_pos: NDArray[np.float64],
               v0: NDArray[np.float64],
               spin: NDArray[np.float64],
    ...
    if extr is None:
        verdict = LBWVerdict("MISSING", np.array([np.nan, np.nan]),
                              None, (0.0, 0.0), 0.0)
    else:
        _, yz = extr
        if param_cov is not None:
            params9 = np.concatenate([release_pos, v0, spin])
            yz_cov = stump_line_covariance(params9, param_cov,
    ...
    return LBWAssessment(
        pitched_in_line, pitched_outside_off, impact_in_line, verdict, out,
    )
