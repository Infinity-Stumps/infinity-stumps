...
def ball_dynamics(
    state: NDArray[np.float64], t: float, spin: NDArray[np.float64], params: BallParams
) -> NDArray[np.float64]:
...
def make_delivery(
    ...
    v0 = speed_mps * np.array([np.cos(ah) * np.cos(av), np.sin(ah), -np.sin(av)])
