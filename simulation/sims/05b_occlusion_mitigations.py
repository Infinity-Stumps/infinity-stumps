...
_spec = importlib.util.spec_from_file_location(
    "sim05",
    __file__.replace("05b_occlusion_mitigations.py", "05_multipath_occlusion.py"),
)
...
def occluded_ranges_with_detection(
    true_pos, anchors, occluders, rng, p_detect: float, p_fp: float
):
...
def run_scenario(scenario_name, seed, p_detect, p_fp, loss, f_scale):
    ...
    sample_p = st[idx, :3]
    meas_t, meas_p = measure_trajectory(
        sample_t, sample_p, ANCHORS_8, seed, p_detect, p_fp
    )
    if len(meas_t) < 20:
        return float("nan"), 0
    ...
    for name, pd, pfp, loss, fs in scenarios:
        ...
        for s in range(n_mc):
            ...
            if not np.isnan(rms):
                rmss.append(rms)
            print(
                f"    [{s + 1}/{n_mc}] fixes={n_fixes:3d}  RMS = "
