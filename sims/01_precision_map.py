...
from cricket_uwb import (ANCHORS_8, RANGE_SIGMA_DEFAULT,
                          solve_position, gdop, add_range_noise)
from cricket_uwb.geometry import gdop_map
from cricket_uwb.plotting import draw_pitch_topdown, draw_pitch_sideview


def monte_carlo_precision(true_pos, anchors=ANCHORS_8, n_trials=500,
...
def main():
    ...
    for name, pos in test_points:
        ...
        print(f"  {name:30s} σ=({s[0]:5.1f},{s[1]:5.1f},{s[2]:5.1f}) "
              f"RMS={rms:5.1f} GDOP={g:.2f}")

    fig = plt.figure(figsize=(15, 9))
    gs = GridSpec(3, 3, figure=fig, hspace=0.4, wspace=0.3)
    ax1 = fig.add_subplot(gs[0, 0]); draw_pitch_topdown(ax1)
    ax1.set_title("Anchor layout (top-down)")
    ax2 = fig.add_subplot(gs[0, 1]); draw_pitch_sideview(ax2)
    ax2.set_title("Anchor layout (side view)")

    ax3 = fig.add_subplot(gs[0, 2])
    names = [r[0].replace(" ", "\n", 1) for r in results]
    rms_vals = [r[2] for r in results]
    cols = ["green" if v < 20 else "orange" if v < 40 else "red"
            for v in rms_vals]
    ax3.barh(range(len(names)), rms_vals, color=cols, edgecolor="k")
    ax3.axvline(20, color="green", ls="--", label="20 mm target")
    ax3.set_yticks(range(len(names))); ax3.set_yticklabels(names, fontsize=8)
    ax3.invert_yaxis(); ax3.set_xlabel("3D RMS (mm)")
    ax3.set_title("Raw precision"); ax3.legend(fontsize=8)
    ...
    for k, z in enumerate([0.5, 1.5, 2.5]):
        ...
        im = ax.imshow(grid, extent=[xs[0], xs[-1], ys[0], ys[-1]],
                       ...
                       vmin=1.0, vmax=4.0)
        ax.scatter(ANCHORS_8[:, 0], ANCHORS_8[:, 1], c="white",
                   s=30, edgecolors="k", zorder=5)
        ax.set_title(f"GDOP at z={z:.1f} m"); ax.set_xlabel("X (m)")
