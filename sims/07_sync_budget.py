...
from cricket_uwb import (ANCHORS_8, RANGE_SIGMA_DEFAULT, fit_trajectory)
...
def generate_sync_obs(t_grid, offsets_rel, sync_rate_hz, seed):
    ...
    for i in range(n_anchors):
        per_anchor[i] = (np.array(per_anchor[i][0]),
                         np.array(per_anchor[i][1]))
    return per_anchor
...
def predict_disciplined(sync_obs_i, t_query, trim_lsb_ppm=TRIM_LSB_PPM,
                         fit_window_s=5.0, rng=None):
...
def solve_tdoa(timestamps_s, anchors, ref_idx=0, x0=None):
    ...
    if x0 is None:
        x0 = np.mean(anchors, axis=0).copy(); x0[2] = 1.0
...
def measure_tdoa(sample_t, sample_p, anchors, sync_obs, offsets_rel,
                  t_grid, mode, rng):
...
def run_one(sync_rate, ppm_mean, ppm_walk, mode, seed):
    ...
    _, offsets_rel = make_anchor_clocks(len(ANCHORS_8), t_grid,
                                         ppm_mean, ppm_walk, seed)
    sync_obs = generate_sync_obs(t_grid, offsets_rel, sync_rate, seed + 1)

    rng = np.random.default_rng(seed + 2)
    meas = measure_tdoa(sample_t, sample_p, ANCHORS_8,
                         sync_obs, offsets_rel, t_grid, mode, rng)
    if len(meas) < 50:
    ...
    fitted, _ = fit_trajectory(sample_t[:len(meas)], meas,
                                sigma_pos=0.3, loss="huber", f_scale=3.0)
...
def main():
    ...
    print(f"  σ per sync obs : {SIGMA_OBS_S*1e12:.1f} ps "
    ...
    for name, rate, ppm, walk, mode in scenarios:
        ...
        for s in range(n_mc):
            ...
            if not np.isnan(f):
                raw_l.append(r); fit_l.append(f)
            print(f"    [{s+1}/{n_mc}] raw={r:8.1f} mm  fit={f:8.1f} mm",
                  flush=True)
        fit_l = np.array(fit_l)
        raw_l = np.array(raw_l)
        results[name] = (raw_l, fit_l)
        print(f"    -> fit mean={fit_l.mean():.1f}  "
    ...
    colors = []
    for name in labels:
        if "perfect" in name: colors.append("#4a4")
        elif "disciplined" in name: colors.append("#48a")
        else: colors.append("#c44")
    bp = ax.boxplot(fits, widths=0.6, patch_artist=True,
