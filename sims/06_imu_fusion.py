...
_spec = importlib.util.spec_from_file_location(
    "sim05", __file__.replace("06_imu_fusion.py",
...
def simulate_imu_body(ts_truth, states_truth, omega_world, R0, seed,
                       perfect=False):
    ...
    bias_a = np.cumsum(
        rng.normal(0, BIAS_A_PSD * np.sqrt(dt_imu), (N, 3)), axis=0)
    bias_g = np.cumsum(
        rng.normal(0, BIAS_G_PSD * np.sqrt(dt_imu), (N, 3)), axis=0)
    accel_body_meas = (accel_body + bias_a
                        + rng.normal(0, SIGMA_A, (N, 3)))
    gyro_body_meas = (gyro_body + bias_g
                       + rng.normal(0, SIGMA_G, (N, 3)))
    return t_imu, accel_body_meas, gyro_body_meas


def strapdown_with_uwb_fusion(t_imu, accel_body, gyro_body,
                                t_uwb, pos_uwb, R0_est, p0, v0,
                                alpha_pos=0.10, alpha_R=0.02):
...
def measure_uwb(sample_t, sample_p, anchors=ANCHORS_8, occlusion=False,
                seed=42):
    rng = np.random.default_rng(seed)
    meas_t, meas_p = [], []
    last = None
    ...
    for t, true_pos in zip(sample_t, sample_p):
        if occlusion:
            ...
            ranges = (np.linalg.norm(anchors - true_pos, axis=1)
                       + rng.normal(0, RANGE_SIGMA_DEFAULT, len(anchors)))
...
def run_scenario(name, occlusion, use_imu, perfect_imu, seed):
    ...
    meas_t, meas_p = measure_uwb(sample_t, sample_p,
    ...
    if not use_imu:
        ...
        fused_pos = strapdown_with_uwb_fusion(
            t_imu, accel_b, gyro_b, meas_t, meas_p,
            R0_est, p0_est, v0_est, alpha_pos=0.5)
        idx_imu = np.clip(np.searchsorted(t_imu, meas_t), 0, len(t_imu)-1)
        fused_at_meas = fused_pos[idx_imu]
        fitted, _ = fit_trajectory(meas_t, fused_at_meas,
...
def main():
    ...
    print(f"  Spin rate   : 25 rev/s = 9000 °/s")
    print(f"  Init attitude error : 0° (perfect — best case)")
    print(f"  Fusion alpha: 0.5 (balanced UWB/IMU)\n", flush=True)
    ...
    for name, rmss in results.items():
        print(f"  {name:32s} {rmss.mean():>6.1f}  "
    ...
    ax.set_ylabel("Fit 3D RMS (mm)"); ax.set_yscale("log")
