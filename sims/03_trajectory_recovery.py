...
def measure_trajectory(
    positions, anchors=ANCHORS_8, sigma=RANGE_SIGMA_DEFAULT, seed=42
):
...
def main():
    ...
    print(
        f"  Raw σ: X={raw_err[:, 0].std() * 1000:.1f} "
    ...
    print(
        f"  Fit σ: X={fit_err[:, 0].std() * 1000:.1f} "
    ...
    fig.suptitle(
        "Sim 03 — Trajectory recovery with physics-constrained fit", fontsize=13
    )
