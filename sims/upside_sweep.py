...
from cricket_uwb.geometry import (PITCH_HL, PAI_X, PAI_Y, STUMP_W,
                                    STUMP_TOP, STUMP_MID)

import importlib.util
_spec = importlib.util.spec_from_file_location(
    "sim05", __file__.replace("upside_sweep.py",
...
def measure_with_detect(true_pos, anchors, occluders, rng,
                         sigma_los, p_detect, p_fp):
...
def measure_multichannel(true_pos, anchors, occluders, rng,
                          sigma_los, p_detect, p_fp,
                          nlos_correlation):
    ...
    for i, anchor in enumerate(anchors):
        ...
        u_ch0 = rng.random()
        u_ch1 = (nlos_correlation * u_ch0
                  + (1 - nlos_correlation) * rng.random())
        ...
        for ch in range(2):
            if not is_blocked:
                ...
                if u < sim05.DROP_PROB:
                    ...
                    ch_vals.append(true_r[i] + rng.uniform(*sim05.NLOS_BIAS)
                                    + rng.normal(0.0, sim05.SIGMA_NLOS))
...
def run_one(anchors, sigma_los, multichannel, nlos_correlation, seed):
    ...
    for t, true_pos in zip(sample_t, sample_p):
        ...
        if multichannel:
            ranges = measure_multichannel(true_pos, anchors, occluders, rng,
                                            sigma_los, P_DETECT, P_FP,
                                            nlos_correlation)
        else:
            ranges = measure_with_detect(true_pos, anchors, occluders, rng,
                                          sigma_los, P_DETECT, P_FP)
    ...
    truth_m = st[idx_m, :3]
    fitted, _ = fit_trajectory(meas_t, meas_p, loss="huber",
...
def sweep(name, anchors, sigma_los, multichannel=False,
           nlos_correlation=0.5):
    rmss = []
    for s in range(N_MC):
        r = run_one(anchors, sigma_los, multichannel,
                     nlos_correlation, seed=900 + s)
        if not np.isnan(r):
            rmss.append(r)
        print(f"    [{s+1}/{N_MC}] {r:6.1f} mm" if not np.isnan(r)
              else f"    [{s+1}/{N_MC}] fail", flush=True)
    return np.array(rmss)


def main():
    print("\nUpside Sweep — 3 levers on sim 05b baseline\n", flush=True)
    print(f"  Baseline: sim 05b config "
          f"(σ={RANGE_SIGMA_DEFAULT*1000:.0f} mm, 8 anchors, single channel)",
          flush=True)
    print(f"  Operating point: TWR + Huber + 85% NLOS detection under occlusion")
    print(f"  All scenarios run at 500 Hz internal (gains scale identically to 100 Hz)\n",
          flush=True)

    results = {}

    print("Baseline reproduction (sim 05b)")
    results["BASELINE"] = sweep("baseline", ANCHORS_BASELINE,
                                  RANGE_SIGMA_DEFAULT)
    print()
    ...
    for rho in [0.3, 0.5]:
        ...
        results[key] = sweep(key, ANCHORS_BASELINE, RANGE_SIGMA_DEFAULT,
                              multichannel=True, nlos_correlation=rho)
    print()
    ...
    for patch, k in zip(bp["boxes"], labels):
        if k == "BASELINE": patch.set_facecolor("#888")
        elif k.startswith("A"): patch.set_facecolor("#4a8")
        elif k.startswith("B"): patch.set_facecolor("#48a")
        elif k.startswith("C"): patch.set_facecolor("#a84")
        patch.set_alpha(0.7)
