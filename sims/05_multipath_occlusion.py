...
def measure_trajectory_occluded(times, positions, anchors=ANCHORS_8, seed=42):
    ...
    return (np.array(meas_t), np.array(meas_p),
            n_drop, n_total, n_skip, per_anchor_drop, per_anchor_samp)
...
def main():
    ...
    meas_t, meas_p, ndrop, ntot, nskip, pa_drop, pa_samp = (
        measure_trajectory_occluded(sample_t, sample_p, seed=42))
    idx_m = np.clip(np.searchsorted(ts_true, meas_t), 0, len(ts_true)-1)
    ...
    print(f"  Per-anchor drop rate   : "
          + ", ".join(f"A{i+1}={100*pa_drop[i]/pa_samp[i]:.0f}%"
                      for i in range(len(pa_drop))))
    print(f"\n  Raw multilateration RMS: {raw_rms:.1f} mm "
          f"(clean-LOS baseline ≈135)")
    print(f"  Physics-fit RMS        : {fit_rms:.1f} mm "
          f"(clean-LOS baseline ≈10)")
    print(f"  Degradation vs clean   : {fit_rms/10:.2f}× ")
    print(f"  Target ≤40 mm (DRS)    : "
          f"{'ACHIEVED ✓' if fit_rms < 40 else 'DEGRADED ✗'}")
    ...
    for s in range(n_mc):
        ...
        print(f"    [{s+1}/{n_mc}] fit RMS = "
              f"{'fail' if np.isnan(fr) else f'{fr:.1f} mm'}", flush=True)
    rmss = np.array(rmss)
    print(f"  MC fit RMS: mean={rmss.mean():.1f}  median={np.median(rmss):.1f} "
    ...
    for t_snap in [0.0, 0.25, 0.5]:
        ...
        ax.add_patch(patches.Circle((bx, by), br, color="orange",
                                     alpha=0.25))
    ax.set_xlim(-12, 13); ax.set_ylim(-3, 2)
    ax.set_xlabel("X (m)"); ax.set_ylabel("Y (m)")
    ax.set_title("Top down — occluders shown"); ax.legend(fontsize=7); ax.grid(alpha=0.3)
    ...
    ax.set_xlim(-12, 13); ax.set_ylim(-0.2, 3)
    ax.set_xlabel("X (m)"); ax.set_ylabel("Z (m)")
    ax.set_title("Side view"); ax.legend(fontsize=7); ax.grid(alpha=0.3)

    ax = fig.add_subplot(gs[0, 2])
    drop_pct = 100 * pa_drop / np.maximum(pa_samp, 1)
    ax.bar(range(1, 9), drop_pct, color="purple", edgecolor="k", alpha=0.7)
    ax.set_xticks(range(1, 9))
    ax.set_xlabel("Anchor ID"); ax.set_ylabel("Drop rate (%)")
    ax.set_title("Per-anchor occlusion"); ax.grid(alpha=0.3, axis="y")
    ...
    ax.set_title("Single delivery error dist"); ax.legend(); ax.grid(alpha=0.3, axis="y")
    ...
    ax.set_xlabel("Fit 3D RMS (mm)"); ax.set_ylabel("Deliveries")
    ax.set_title("MC over 20 deliveries"); ax.legend(); ax.grid(alpha=0.3, axis="y")
    ...
    fig.suptitle("Sim 05 — Trajectory recovery with cylindrical occlusion",
                 fontsize=13)
