...
def ground_multipath_bias(anchor_pos, ball_pos, refl_coef, rng):
    ...
    if d_h < 0.5:
        return 0.0
    delta_d = (np.sqrt(d_h**2 + (h_b + h_a)**2)
                - np.sqrt(d_h**2 + (h_b - h_a)**2))
...
def measure_trajectory_multipath(sample_t, sample_p, anchors=ANCHORS_8,
                                  refl_coef=REFL_COEF_DRY, seed=42):
...
def main():
    ...
    for name, rc in scenarios:
        ...
        for s in range(n_mc):
            ...
            raws.append(r); fits.append(f)
            print(f"    [{s+1}/{n_mc}] raw={r:6.1f}  fit={f:6.1f} mm",
                  flush=True)
        results[name] = (np.array(raws), np.array(fits))
        f_arr = results[name][1]
        print(f"    -> fit mean={f_arr.mean():.1f}  "
              f"median={np.median(f_arr):.1f}  "
              f"p95={np.percentile(f_arr,95):.1f}\n", flush=True)

    print("\nSUMMARY")
    print("-" * 70)
    print(f"  {'scenario':30s} {'raw mean':>10s} {'fit mean':>10s} "
    ...
    for name, (raw, fit) in results.items():
        ...
        print(f"  {name:30s} {raw.mean():>9.1f}  {fit.mean():>9.1f}  "
    ...
    ax.scatter(st_noisy[:, 0], st_noisy[:, 2], c="red", s=3, alpha=0.4,
    ...
    ax.set_title("Side view — dry pitch"); ax.legend(fontsize=8)
    ax.grid(alpha=0.3); ax.set_xlim(-11, 11); ax.set_ylim(-0.2, 3)
    ...
    for i in range(8):
        ...
        if np.abs(bz).max() > 1:
            ax.plot(st_t*1000, bz, lw=0.8,
                    label=f"A{i+1} (z={ANCHORS_8[i,2]:.2f}m)")
        else:
            ax.plot(st_t*1000, bz, lw=0.5, alpha=0.3, color="grey")
    ax.set_xlabel("Time (ms)"); ax.set_ylabel("Ground-bounce bias (mm)")
